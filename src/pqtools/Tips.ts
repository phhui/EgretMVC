class Tips extends egret.Sprite {
    ...
    public show(text: string, panel: any, x: number, y: number) {
        this.str = text;
        this.x = x;
        this.y = y;
        panel.addChild(this);
    ...
    private init() {
        let sp:egret.Sprite=UiHelper.createRoundRect(0,0,this.stage.stageWidth-100,50,1,0x000000,0x000000,0.2,10);
        sp.x=50;
        this.addChild(sp);
        ...
        TweenLite.to(this,300,{y:ty,alpha:1,onComplete:this.removeAction,target:this});
    }
    private removeAction(){
        let ty:number=this.y-200;
        TweenLite.to(this,300,{y:ty-200,alpha:0,visible:false,delay:800,onComplete:this.removeThis,target:this});
    }
    private removeThis(){
